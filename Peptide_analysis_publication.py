...
def surrounding_cys(dictionary):
    '''
    Used to get single motifs in a defined range around the cysteine, use the function below instead which is more
    general
    '''
    near_cys = dict()
    for peptide in dictionary:
...
def surrounding_cys_pub(dictionary, filename, greater_than):
    '''
    this function was used to get the 6mer and 7mer motifs with cysteines in them
    '''
    motif_dict = dict()
    for pep in dictionary:
        pos = [m.start() for m in re.finditer('C', pep)]
        for i in pos:
            for j in range(i-5,i+1):
                if len(pep[j:j+6]) != 6:
                    pass
                else:
                    if pep[j:j+6] in motif_dict:
                        motif_dict[pep[j:j+6]] += dictionary[pep]
                    else:
                        motif_dict[pep[j:j+6]] = dictionary[pep]
    if greater_than:
        find = find_values_greater_than(motif_dict, greater_than)  #narrow the motifs you are interested in
    if filename:
        F1 = open(filename, 'w') #write the motifs you want to a file
        for motif in find:
            F1.write(motif + ',' + str(find[motif]) + '\n')
        F1.close()

    return motif_dict

def flanking_FCPF_pub(dictionary,seq=re.compile('[FILVWY]CP[FILVWY]')):
    '''
    Used to get the frequencies of the two residues flanking a 'FCPF' motif
    the motif you are interested in can change, just pass a different string to seq above
    '''
    flank_FCPF_dic = dict()

    for pep in dictionary:
        if re.search(seq,pep):
            motif = re.search(seq,pep).group(0)
            ind = pep.find(motif)
            long_mot = pep[ind-2:ind+6]
            if long_mot not in flank_FCPF_dic:
                flank_FCPF_dic[long_mot] = dictionary[pep]
            else:
                flank_FCPF_dic[long_mot] += dictionary[pep]

    flank_FCPF_hist = {x:{z:0 for z in 'ACDEFGHIKLMNPQRSTVWY'} for x in '1278'}

    for pep in flank_FCPF_dic:
        for i in range(len(pep)):
            if str(i+1) in flank_FCPF_hist:
                try:
                    flank_FCPF_hist[str(i+1)][pep[i]] += 1
                except:
                    pass
    return flank_FCPF_hist, flank_FCPF_dic
...
# NoDTT_file = open('NoDTT_peps_final.txt')
# DTT_library = read_file(DTT_file)
# lib_DTT_C, lib_no_C_DTT = cys_dict(DTT_library)
#
# NoDTT_library = read_file(NoDTT_file)
# lib_NoDTT_C, lib_no_C_DTT = cys_dict(NoDTT_library)
#
# DTT_file.close()
# NoDTT_file.close()
...
# code run in the shell
# import re
# seq = re.compile('[FILVWY]CP[FILVWY]')
# NoDTT_FCPFmotif = dict()
# for pep in NoDTT_library:
#     if re.search(seq,pep):
#         motif = re.search(seq,pep).group(0)
#         if motif not in NoDTT_FCPFmotif:
#             NoDTT_FCPFmotif[motif] = 0
#         else:
#             NoDTT_FCPFmotif[motif] += 1
#
# F1 = open('Not Reduced FCPF-type motifs.csv', 'w')
# for motif in NoDTT_FCPFmotif:
#     F1.write(motif)
#     F1.write(',')
#     F1.write(str(NoDTT_FCPFmotif[motif]))
#     F1.write('\n')
#
# fours_red = dict()
# for pep in DTT_library:
#     for i in range(1,len(pep)-4):
#          if pep[i:i+4] not in fours_red:
#             fours_red[pep[i:i+4]] = 1
#          else:
#              fours_red[pep[i:i+4]] += 1
